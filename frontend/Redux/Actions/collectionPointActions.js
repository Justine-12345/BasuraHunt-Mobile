...
export const getTodayCollectionPointList = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${baseURL}/collectionPoint/today`, config)
...
export const getUpcomingCollectionPointList = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${baseURL}/collectionPoint/upcoming`, config)
...
export const addCollectionMasses = (id, collectionPointData) => async (dispatch) => {
    try {

        dispatch({ type: ADD_COLLECTION_MASSES_REQUEST })
        ...
        const { data } = await axios.put(`/api/v1/collectionPoint/masses/${id}`, collectionPointData, config)

        dispatch({
            type: ADD_COLLECTION_MASSES_SUCCESS,
            payload: data.success
        })

    } catch (error) {
        dispatch({
            type: ADD_COLLECTION_MASSES_FAIL,
            payload: error.response.data.message
        })
    }
}
